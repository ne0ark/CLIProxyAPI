...
const (
	redisProtocolMaxArrayLength     = 32
	redisProtocolMaxBulkStringBytes = 64 * 1024
	redisProtocolMaxLineBytes       = 4 * 1024
)
...
func (s *Server) handleRedisConnection(conn net.Conn, reader *bufio.Reader) {
	...
	releaseRedisConnection := s.trackRedisConnection(conn, localClient)
	defer releaseRedisConnection()
	...
	for {
		...
		if authed && !s.redisManagementAllowedForClient(localClient) {
		...
		case "LPOP", "RPOP":
			...
			queue := redisqueue.DefaultQueue()
			if s != nil && s.usageQueue != nil {
				queue = s.usageQueue
			}
			var items [][]byte
			if cmd == "RPOP" {
				items = queue.PopNewest(count)
			} else {
				items = queue.PopOldest(count)
			}
			if hasCount {
...
func (s *Server) redisManagementAllowedForClient(localClient bool) bool {
	if s == nil {
		return false
	}
	if localClient {
	...
	return s.cfg.RemoteManagement.AllowRemote
}
...
func readRESPArray(reader *bufio.Reader) ([]string, error) {
	...
	if count > redisProtocolMaxArrayLength {
		return nil, fmt.Errorf("protocol error")
	}
...
func readRESPBulkString(reader *bufio.Reader) (string, error) {
	...
	if length > redisProtocolMaxBulkStringBytes {
		return "", fmt.Errorf("protocol error")
	}
...
func readRESPLine(reader *bufio.Reader) (string, error) {
	...
	for {
		...
		if len(line) > redisProtocolMaxLineBytes {
			return "", fmt.Errorf("protocol error")
		}
		if err == nil {
			break
	...
	result := strings.TrimSuffix(string(line), "\n")
