...
func TestReadRESPArrayRejectsOversizedArrayLength(t *testing.T) {
	reader := bufio.NewReader(strings.NewReader("*33\r\n"))

	if _, err := readRESPArray(reader); err == nil || err.Error() != "protocol error" {
		t.Fatalf("readRESPArray() error = %v, want protocol error", err)
	}
}

func TestReadRESPArrayRejectsOversizedBulkStringLength(t *testing.T) {
...
func TestRedisProtocol_AUTH_And_PopContracts(t *testing.T) {
	...
	server.usageQueue.Enqueue([]byte("a"))
	server.usageQueue.Enqueue([]byte("b"))
	server.usageQueue.Enqueue([]byte("c"))
...
func TestRedisProtocol_RemoteSessionRevokedWhenAllowRemoteIsDisabled(t *testing.T) {
	passwordHash, err := bcrypt.GenerateFromPassword([]byte("remote-password"), bcrypt.DefaultCost)
	if err != nil {
		t.Fatalf("failed to hash remote management password: %v", err)
	}
	...
	if errWrite := writeTestRESPCommand(clientConn, "AUTH", "remote-password"); errWrite != nil {
		t.Fatalf("failed to write AUTH command: %v", errWrite)
	}
	...
	updated := *server.cfg
	updated.RemoteManagement.AllowRemote = false
	server.UpdateClients(&updated)

	waitForRESPConnectionClose(t, clientConn, reader, 2*time.Second)
}

func TestRedisProtocol_FailedAuthRevokesAuthenticatedSession(t *testing.T) {
	const managementPassword = "test-management-password"
	...
	server.usageQueue.Enqueue([]byte("item"))
	...
	if msg, err := readTestRESPError(reader); err != nil {
		...
		t.Fatalf("unexpected LPOP response after failed AUTH: %q", msg)
	}
}
