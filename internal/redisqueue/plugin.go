...
func (p *usageQueuePlugin) HandleUsage(ctx context.Context, record coreusage.Record) {
	...
	usageQueue := FromContext(ctx)
	if usageQueue == nil && ctx != nil {
		if ginCtx, ok := ctx.Value("gin").(*gin.Context); ok && ginCtx != nil && ginCtx.Request != nil {
			usageQueue = FromContext(ginCtx.Request.Context())
		}
	}
	if usageQueue == nil {
		usageQueue = DefaultQueue()
	}
	if usageQueue == nil || !usageQueue.Enabled() || !usageQueue.RecordingEnabled() {
		return
	}
