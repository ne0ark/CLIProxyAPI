...
func TestUsageQueuePluginPayloadPreservesAdditionalModelFlag(t *testing.T) {
	withEnabledQueue(t, func() {
		ginCtx := newTestGinContext(t, http.MethodPost, "/v1/responses", http.StatusOK)
		ctx := context.WithValue(context.Background(), "gin", ginCtx)
		...
		payload := popSinglePayload(t)
		requireBoolField(t, payload, "additional_model", true)
	})
}

func TestUsageQueuePluginPayloadPrefersRecordSnapshotFields(t *testing.T) {
	withEnabledQueue(t, func() {
		ginCtx := newTestGinContext(t, http.MethodGet, "/wrong", http.StatusOK)
		internallogging.SetGinRequestID(ginCtx, "wrong-gin-request-id")
		ctx := context.WithValue(context.Background(), "gin", ginCtx)
		...
		payload := popSinglePayload(t)
		requireStringField(t, payload, "endpoint", "POST /expected")
		requireStringField(t, payload, "request_id", "expected-request-id")
		requireBoolField(t, payload, "failed", true)
	})
}

func TestUsageQueuePluginPayloadAllowsNilContextWithoutRequestIDFallback(t *testing.T) {
	withEnabledQueue(t, func() {
		plugin := &usageQueuePlugin{}
		...
		requireStringField(t, payload, "endpoint", "")
	})
}
