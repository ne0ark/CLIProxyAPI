...
func TestAntigravityShouldDisableCredits_IgnoresStructuredQuota429(t *testing.T) {
	body := []byte(`{
		"error": {
			"code": 429,
			"message": "Resource has been exhausted for this model.",
			"status": "RESOURCE_EXHAUSTED",
			"details": [
				{
					"@type": "type.googleapis.com/google.rpc.ErrorInfo",
	...
	if antigravityShouldDisableCredits(http.StatusTooManyRequests, body) {
		t.Fatal("antigravityShouldDisableCredits() = true, want false for structured quota exhaustion")
	}
}
...
func TestEnsureAccessToken_WarmTokenRefreshesKnownCreditsHint(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)
	...
	hint, ok := cliproxyauth.GetAntigravityCreditsHint(auth.ID)
	if !ok {
		t.Fatal("expected refreshed credits hint lookup to succeed")
	}
	if !hint.Available {
		t.Fatalf("hint.Available = %v, want true", hint.Available)
	}
...
func TestEnsureAccessToken_WarmTokenRetriesHintRefreshAfterFailure(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)
	...
	for time.Now().Before(deadline) {
		...
		currentCall := callCount
		mu.Unlock()
		if currentCall >= 2 && cliproxyauth.HasKnownAntigravityCreditsHint(auth.ID) {
			hint, ok := cliproxyauth.GetAntigravityCreditsHint(auth.ID)
			if ok && hint.Available {
				return
			}
	...
	currentCall := callCount
	mu.Unlock()
	t.Fatalf("expected second hint refresh attempt and available hint, calls=%d hintKnown=%v", currentCall, cliproxyauth.HasKnownAntigravityCreditsHint(auth.ID))
}

func TestMarkAntigravityCreditsPermanentlyDisabledUpdatesHint(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)

	auth := &cliproxyauth.Auth{ID: "auth-disabled-hint"}
	cliproxyauth.SetAntigravityCreditsHint(auth.ID, cliproxyauth.AntigravityCreditsHint{
		Known:           true,
		Available:       true,
		CreditAmount:    100,
	...
	if antigravityAuthHasCredits(auth) {
		t.Fatal("antigravityAuthHasCredits() = true, want false after permanent disable")
	}
	hint, ok := cliproxyauth.GetAntigravityCreditsHint(auth.ID)
	if !ok {
		t.Fatal("expected disabled credits hint lookup to succeed")
	}
	if !hint.Known || hint.Available {
		t.Fatalf("hint = %+v, want Known=true and Available=false", hint)
	}
}

func TestUpdateAntigravityCreditsBalance_NoGoogleOneCreditsClearsAvailability(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)
	...
	antigravityCreditsBalanceByAuth.Store(auth.ID, antigravityCreditsBalance{
		CreditAmount:    100,
		MinCreditAmount: 50,
		Known:           true,
	})
	...
	ctx := context.WithValue(context.Background(), "cliproxy.roundtripper", roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
			Body:       io.NopCloser(strings.NewReader(`{"paidTier":{"id":"tier-1","availableCredits":[{"creditType":"OTHER_CREDIT","creditAmount":"5","minimumCreditAmountForUsage":"1"}]}}`)),
		}, nil
	}))
	...
	if antigravityAuthHasCredits(auth) {
		t.Fatal("antigravityAuthHasCredits() = true, want false when GOOGLE_ONE_AI credits are absent")
	}
	hint, ok := cliproxyauth.GetAntigravityCreditsHint(auth.ID)
	if !ok {
		t.Fatal("expected credits hint lookup to succeed")
	...
	if _, ok := antigravityCreditsBalanceByAuth.Load(auth.ID); ok {
		t.Fatal("expected cached credits balance to be cleared")
	}
}
