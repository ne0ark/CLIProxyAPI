...
func assertCloakedCacheControlRewrite(t *testing.T, body []byte) {
	...
	if len(system) != 9 {
		t.Fatalf("expected 9 cloaked system blocks, got %d", len(system))
	}
	if system[0].Get("cache_control").Exists() {
		t.Fatalf("system.0.cache_control should be stripped for cloaked payloads")
	}
	for i := 1; i <= 6; i++ {
		if system[i].Get("cache_control").Exists() {
			t.Fatalf("system.%d.cache_control should be stripped after cache-control limit enforcement", i)
		}
	}
	for _, idx := range []int{7, 8} {
		if system[idx].Get("cache_control.type").String() != "ephemeral" {
			t.Fatalf("system.%d.cache_control.type = %q, want %q", idx, system[idx].Get("cache_control.type").String(), "ephemeral")
		}
	}
