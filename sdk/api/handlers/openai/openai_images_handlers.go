...
func cloneSSEFrame(frame []byte) []byte {
	if len(frame) == 0 {
		return nil
	}
...
func (a *sseFrameAccumulator) AddChunk(chunk []byte) [][]byte {
	...
	for {
		...
		frames = append(frames, cloneSSEFrame(a.pending[:frameLen]))
	...
	frames = append(frames, cloneSSEFrame(a.pending))
	a.pending = a.pending[:0]
	return frames
}
...
func resolveImageToolModel(modelName string) string {
	modelName = strings.TrimSpace(modelName)
	if modelName == "" || strings.EqualFold(modelName, defaultImagesToolModel) {
	...
	for _, candidate := range []string{info.ID, info.Name, info.Version, info.DisplayName} {
		if strings.EqualFold(strings.TrimSpace(candidate), defaultImagesToolModel) {
			return defaultImagesToolModel
		}
	}
...
func (h *OpenAIAPIHandler) ImagesGenerations(c *gin.Context) {
	...
	requestedImageModel := strings.TrimSpace(gjson.GetBytes(rawJSON, "model").String())
	if requestedImageModel == "" {
		requestedImageModel = defaultImagesToolModel
	}
	imageToolModel := resolveImageToolModel(requestedImageModel)
	...
	tool, _ = sjson.SetBytes(tool, "model", imageToolModel)
...
func (h *OpenAIAPIHandler) imagesEditsFromMultipart(c *gin.Context) {
	if c.Request != nil {
		if c.Request.ContentLength > imagesMultipartMaxBytes {
			c.JSON(http.StatusRequestEntityTooLarge, handlers.ErrorResponse{
				Error: handlers.ErrorDetail{
					Message: "Invalid request: multipart body exceeds limit",
					Type:    "invalid_request_error",
				},
			})
			return
		...
		if c.Request.Body != nil {
			c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, imagesMultipartMaxBytes)
		}
	}
	...
	requestedImageModel := strings.TrimSpace(c.PostForm("model"))
	if requestedImageModel == "" {
		requestedImageModel = defaultImagesToolModel
	}
	imageToolModel := resolveImageToolModel(requestedImageModel)
...
func (h *OpenAIAPIHandler) streamImagesFromResponses(c *gin.Context, responsesReq []byte, executionModel, requestedModel, responseFormat string, streamPrefix string) {
