...
const antigravityCreditsHintMaxAge = 10 * time.Minute
...
func antigravityCreditsHintIsStale(hint AntigravityCreditsHint, now time.Time) bool {
	if hint.UpdatedAt.IsZero() {
		return true
	}
	return now.Sub(hint.UpdatedAt) >= antigravityCreditsHintMaxAge
}
